...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ...
        await message.reply_text(text=f"<b>ʜᴇʏ {user} 🫶🏻 ,\n<blockquote>ʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ🚫\nʙᴏᴛ ᴡᴏʀᴋ's ᴏɴʟʏ ɪɴ ɢʀᴏᴜᴘ✅\nᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</blockquote>\n\n<blockquote>நீங்கள் இங்கிருந்து திரைப்படங்களைப் பெற முடியாது🚫\nகுழுவில் மட்டும் நான் வேலை செய்வேன் ✅\nஇங்கே கோரிக்கையை கிளிக் செய்யவும் கீழே உள்ள பட்டனை 👇</blockquote></b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Mᴏᴠɪᴇ Gʀᴏᴜᴘ 𝟷 🔎", url=f"https://t.me/+to0CS22_eqQ3YWM9")],[InlineKeyboardButton("(Oɴʟʏ Tᴀᴍɪʟ Mᴏᴠɪᴇ) Gʀᴏᴜᴘ 2 🔎", url=f"https://t.me/+sVDLf3pVUi8yNGI1")]]))
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton(text=f"📃 Pᴀɢᴇ {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages"), InlineKeyboardButton("Nᴇxᴛ", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    if offset != "":
        try:
            if settings['max_btn']:
                btn.append(
                    [InlineKeyboardButton(text=f"📃 Pᴀɢᴇ 1/{math.ceil(int(total_results) / 10)}", callback_data="pages"), InlineKeyboardButton(text="Nᴇxᴛ",callback_data=f"next_{req}_{key}_{offset}")]
                )
