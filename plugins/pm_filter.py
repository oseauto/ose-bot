        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    ai_search = True
                    reply_msg = await message.reply_text(f"")
...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ai_search = True
        reply_msg = await bot.send_message(message.from_user.id, f"", reply_to_message_id=message.id)
    ...
    if settings['button']:
        ...
        btn.insert(0, 
            [
                ...
                InlineKeyboardButton("Eᴘɪsᴏᴅᴇs 🗒️", callback_data=f"episodes#{key}"),
                InlineKeyboardButton("Sᴇᴀsᴏɴs 📖",  callback_data=f"seasons#{key}")
            ]
        )
        btn.insert(0, [
            InlineKeyboardButton("Sᴇɴᴅ Aʟʟ 📤", callback_data=f"sendfiles#{key}"),
            InlineKeyboardButton("Lᴀɴɢᴜᴀɢᴇs 🗣️", callback_data=f"languages#{key}"),
    ...
    if gl == False:
        ...
        if k == False:
            ...
            if files:
                ...
                ai_search = True
                reply_msg = await query.message.edit_text(f"")
